...
class CognitoUser:
    _jwt = current_cognito_jwt
    _user = current_user
    _cog = _cog

    @classproperty
    def user(cls):
        return cls._user._get_current_object()

    @classproperty
    def jwt(cls):
        return cls._jwt._get_current_object()

    @classproperty
    def is_anonymous(cls):
        return not bool(cls.jwt)
    ...
    @classproperty
    def is_in_database(cls):
        return bool(cls.user)

    @classproperty
    def email(cls):
        return cls._jwt.get("email")

    @classproperty
    def subject(cls):
        return cls._jwt.get("subject")


# user = CurrentUser
cognito_user = CognitoUser
# current_cognito_jwt = CurrentUser.jwt
# current_jwt = CurrentUser.jwt
...
class CognitoAuth(object):
    ...
    def init_app(self, app, identity_handler=None):
        ...
        self.userpool_id = self._get_required_config(
            app, "COGNITO_USERPOOL_ID"
        )
        self.jwt_header_name = self._get_required_config(
            app, "COGNITO_JWT_HEADER_NAME"
        )
        ...
        self.check_expiration = app.config.get(
            "COGNITO_CHECK_TOKEN_EXPIRATION", True
        )
    ...
    def get_token(self):
        ...
        if not auth_header_value or "null" in auth_header_value:
        ...
        if parts[0].lower() != auth_header_prefix.lower():
            ...
            raise CognitoAuthError(
                "Invalid Cognito JWT header", "Token missing"
            )
        elif len(parts) > 2:
            raise CognitoAuthError(
...
load_cognito_tokens = load_jwt_tokens
